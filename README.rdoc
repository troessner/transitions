...
Use the (surprise ahead) `current_state` method, in case you didn't set one explicitly you'll get the state back that you defined a initial state.

  >> Product.new.current_state
  => :available

You can also set a new state explicitly `update_current_state(new_state, persist = true / false)` but you should never do this unless you really know what you're doing and why - rather use state transitions (see below).
